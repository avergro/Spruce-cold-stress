#' ---
#' title: "Arabidopsis RNA acclimation data analysis (phase1 data to compare against Spruce)"
...
#' Create a result dir
system("mkdir ANALYSIS_LEAVES")
outdir <- file.path("ANALYSIS_LEAVES","manuscript_version")
dir.create(outdir,showWarnings=FALSE)

#' Read the HTSeq files in a matrix
res <- mclapply(dir("HTSeq_including_all_replicate_samples",pattern="*.txt",full.names=TRUE),function(fil){
  read.delim(fil,header=FALSE,stringsAsFactors=FALSE)
},mc.cores=max(mcaffinity()))

names(res) <- sub("arabidopsis_","",sub(".txt","",dir("HTSeq_including_all_replicate_samples",pattern="*.txt")))
...
write.csv(count.table,file.path(outdir,"raw-unormalised-data_15_samples_ATH_Leaves_Phase1.csv"))
...
plot.multidensity(log10(count.table),
                  col=pal[as.integer(samples$SampleName)],
                  legend.x="topright",
...
#In total we have 27416 gene models
#######################################################################
...
#' ## Raw data tissues
#' analyizing raw counts from phase1 (5°C) 
#' We will keep genes with 2 or more reads in at least 3 replicates
head(samples)
#' In each condition we have 3 reps
#' We considered "outliers" for DE analysis the samples: control1 and 3d_2
#' In these time of treatment we removed 1 replicate. So we have 2 replicates in these cases. 
#' We used the seame criterium than for Spruce data. So we filter using 2 like minimun amount of reads and this must happend in 3 replicates
...
write.table(survivors, file="Survivors_genes_ATH_leaves_phase1_20106.txt", quote = F, row.names = F, col.names = F)
